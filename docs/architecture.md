...
| `get_community_package_tools` | downloads a Community package (no internalize, no push) and returns its real `tools/chocolateyInstall.ps1`/`chocolateyUninstall.ps1` plus a best-effort `detectedSilentArgs` extracted from it; tries `<id>.install` automatically if the bare id is a tools-less metapackage | none (read-only) |
