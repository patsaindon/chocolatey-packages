...
- **Internal packages** are [AU](https://github.com/majkinetor/AU) (Chocolatey Automatic Packages): each one checks its own vendor's release page directly via `update.ps1`. Before packing, each package's own `au_BeforeUpdate` hook mirrors the real binary into Nexus generic storage (scanned first) and rewrites the URL to point there instead of the vendor — deliberately not Chocolatey's own Package Internalizer, which embeds a Business/MSP license requirement into every consuming endpoint. `update-internal-packages.yml` runs the same daily-diff model against them; `test-internal-packages.yml` validates changes (including the real mirror step) on a PR without ever touching a feed.
...
**Every internal AU package** (not just paywalled ones) also mirrors its own binary into this same repository automatically — see `docs/architecture.md § 9.3`. That needs *write* access, scoped separately from the anonymous-read setup above (never reuse it, and never use `admin`):

```
curl -s -u admin:ADMIN_PW -X POST http://localhost:8082/service/rest/v1/security/roles \
  -H "Content-Type: application/json" \
  -d '{"id":"generic-hosted-write","name":"generic-hosted-write","privileges":["nx-repository-view-raw-generic-hosted-read","nx-repository-view-raw-generic-hosted-add","nx-repository-view-raw-generic-hosted-edit","nx-repository-view-raw-generic-hosted-browse"]}'

curl -s -u admin:ADMIN_PW -X POST http://localhost:8082/service/rest/v1/security/users \
  -H "Content-Type: application/json" \
  -d '{"userId":"au-mirror","firstName":"AU","lastName":"Mirror","emailAddress":"au-mirror@example.org","password":"CHANGE_ME","status":"active","roles":["generic-hosted-write"]}'
```

Then set `NEXUS_MIRROR_BASE_URL`/`NEXUS_MIRROR_REPOSITORY`/`NEXUS_GENERIC_WRITE_TOKEN` (`au-mirror:CHANGE_ME`) wherever `internal/*/update.ps1` runs (both `update-internal-packages.yml` and `test-internal-packages.yml`).
