...
| `scaffold_internal_package` | copies `internal/_template/`, fills in placeholders; pass `evergreen_app_name` to seed a real `au_GetLatest` instead of a generic placeholder — also picks (and bakes into the generated filter) whichever variant's `InstallerType`/`Type` actually matches the requested `package_kind`, and propagates the real file type (`fileType`, defaulting `silentArgs` to `/qn /norestart` for a real `.msi`) instead of leaving the template's `exe`/`'/S'` defaults regardless of what was selected — both verified end-to-end against real apps (Alacritty: picks the `.msi` installer, not its portable `.exe`; Rufus: picks a real `x86` build over its `ARM64`-only fallback, since Rufus publishes no `x64` variant at all); pass `vendor` (same slug as the two tools above) to auto-fill `silent_args`/`source_url` from `knowledge/<vendor>.yml` when you haven't found a fresher value yourself — an explicit `silent_args`/`source_url` param always wins over the knowledge base. Also seeds the nuspec's `<title>`/`<authors>`/`<projectUrl>`/`<tags>` from evergreen's own app index, or from explicit `title`/`vendor_name`/`source_url` params, instead of leaving them as the bare package id / empty. Pass all three `nexus_generic_base_url`/`nexus_generic_repository`/`nexus_generic_path_prefix` instead, for paywalled software, to generate an `au_GetLatest` that reads the latest binary a human already uploaded to Nexus rather than scraping a vendor page this automation can't log into — see [../scripts/Get-NexusGenericLatestAsset.ps1](../scripts/Get-NexusGenericLatestAsset.ps1), verified end-to-end against a real Nexus instance (Docker; see the top-level README). Pass `dependencies` (an array of `{id, version?}`) to fill in the nuspec's `<dependencies>` — the same element `Get-PromotionCandidates.ps1` already reads to carry a dependency into a promotion batch, verified end-to-end against a real generated nuspec. Pass `package_kind: 'portable'` (plus optionally `binary_name`) for a release that's just a standalone CLI binary with no installer at all — generates a `Get-ChocolateyWebFile`-based `chocolateyinstall.ps1` instead of the `Install-ChocolateyPackage` one, verified end-to-end against a real binary (CodeGraphContext's `cgc-windows.exe`: real download, checksum match, and the tool actually runs after being placed under `tools\`) | local working tree only |
