...
function global:au_GetLatest {
    ...
    # for 'AnacondaMiniconda' — verify the filter below still matches what
    # this package actually wants to ship (was: Architecture=x86).
    $releases = "https://evergreen-api.stealthpuppy.com/app/AnacondaMiniconda"
    $variants = Invoke-RestMethod -Uri $releases -UserAgent "chocolatey-packages-mcp-server"
    $latest = $variants | Where-Object { $_.Architecture -eq 'x86' } | Select-Object -First 1
    if (-not $latest) { throw "No matching evergreen-api variant found for AnacondaMiniconda." }
    ...
    if ($version -notmatch '^\d+(\.\d+){0,3}(-.+)?$') {
        throw "Sanitized version '$version' has more than 4 numeric segments (NuGet/choco's limit) — adjust this au_GetLatest's version handling for AnacondaMiniconda's actual version format."
    }
