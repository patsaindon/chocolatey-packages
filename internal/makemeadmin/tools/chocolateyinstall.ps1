$ErrorActionPreference = 'Stop'

$toolsPath = Split-Path $MyInvocation.MyCommand.Definition
# url/checksum are left empty here on purpose: update.ps1's au_SearchReplace
# fills them in from au_GetLatest's result at update time. Committing this
# file with empty url/checksum is the normal, expected state for an AU
# package — do not fill them in by hand.
$packageArgs = @{
  packageName    = 'makemeadmin'
  fileType       = 'msi'
  url            = ''
  softwareName   = 'makemeadmin*'         # part of/whole software name, for uninstall registry lookup
  checksum       = ''
  checksumType   = 'sha256'
  silentArgs     = '/qn /norestart /l*v "%TEMP%\%chocolateyPackageName%.%chocolateyPackageVersion%.MsiInstall.log"'
  validExitCodes = @(0)
}

Install-ChocolateyPackage @packageArgs
