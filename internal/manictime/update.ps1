...
function global:au_GetLatest {
    ...
    # for 'ManicTimeClient' — verify the filter below still matches what
    # this package actually wants to ship (was: no distinguishing fields on this app's evergreen entries).
    $releases = "https://evergreen-api.stealthpuppy.com/app/ManicTimeClient"
    $variants = Invoke-RestMethod -Uri $releases -UserAgent "chocolatey-packages-mcp-server"
    $latest = $variants | Where-Object { $true } | Select-Object -First 1
    if (-not $latest) { throw "No matching evergreen-api variant found for ManicTimeClient." }
    ...
    if ($version -notmatch '^\d+(\.\d+){0,3}(-.+)?$') {
        throw "Sanitized version '$version' has more than 4 numeric segments (NuGet/choco's limit) — adjust this au_GetLatest's version handling for ManicTimeClient's actual version format."
    }
