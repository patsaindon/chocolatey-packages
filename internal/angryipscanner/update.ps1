...
function global:au_GetLatest {
    ...
    # for 'angryziberAngryIPScanner' — verify the filter below still matches what
    # this package actually wants to ship (was: Architecture=x64, InstallerType=Default).
    $releases = "https://evergreen-api.stealthpuppy.com/app/angryziberAngryIPScanner"
    ...
    if (-not $latest) { throw "No matching evergreen-api variant found for angryziberAngryIPScanner." }
    ...
    if ($version -notmatch '^\d+(\.\d+){0,3}(-.+)?$') {
        throw "Sanitized version '$version' has more than 4 numeric segments (NuGet/choco's limit) — adjust this au_GetLatest's version handling for angryziberAngryIPScanner's actual version format."
    }
