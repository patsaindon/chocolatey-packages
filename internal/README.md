...
**When the software is paywalled** (a vendor login gates the real download, which this automation must never hold credentials for), `au_GetLatest` can instead read the latest binary a human already uploaded to a Nexus generic/raw-format hosted repository — see [scripts/Get-NexusGenericLatestAsset.ps1](../scripts/Get-NexusGenericLatestAsset.ps1) and `scaffold_internal_package`'s `nexus_generic_*` parameters ([mcp-server/README.md](../mcp-server/README.md)). Verified end-to-end against a real Nexus instance (see the top-level README's Docker recipe) — including the Nexus-side requirement it surfaced: the target repository needs to be readable by whatever downloads the returned URL at install time, since `Get-ChocolateyWebFile` has no credential of its own to send. See [docs/architecture.md § 6.8](../docs/architecture.md#68-mcp-server--agent-driven-package-creation) for the tested, narrowly-scoped way to configure that.
...
- **On a schedule**: [.github/workflows/update-internal-packages.yml](../.github/workflows/update-internal-packages.yml) lints every package, then runs `update_all.ps1` (AU's `updateall`) with push disabled. Each package's own `au_BeforeUpdate` hook (defined in `internal/_template/update.ps1`) mirrors the real vendor binary into Nexus generic storage first — scanning it via `scripts/scan-package.ps1 -RawFilePath` before it ever gets uploaded — and rewrites the URL/checksum AU packs with to point at that mirrored copy instead of the vendor. The packed package therefore already only references Nexus, so it's pushed to staging as-is; nothing left to internalize. This deliberately does **not** use `choco download --internalize` (which internal AU packages used to go through) — that embeds a Chocolatey Business/MSP license requirement into the resulting package, confirmed by inspecting a real internalized package's own script, which the Nexus-mirror approach avoids entirely. See `docs/architecture.md` section 9.3 for the full design and its real-Nexus verification.
