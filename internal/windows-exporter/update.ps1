...
function global:au_GetLatest {
    ...
    # for 'PrometheusWindowsExporter' — verify the filter below still matches what
    # this package actually wants to ship (was: Architecture=x64, InstallerType=Default).
    $releases = "https://evergreen-api.stealthpuppy.com/app/PrometheusWindowsExporter"
    ...
    if (-not $latest) { throw "No matching evergreen-api variant found for PrometheusWindowsExporter." }
    ...
    if ($version -notmatch '^\d+(\.\d+){0,3}(-.+)?$') {
        throw "Sanitized version '$version' has more than 4 numeric segments (NuGet/choco's limit) — adjust this au_GetLatest's version handling for PrometheusWindowsExporter's actual version format."
    }
