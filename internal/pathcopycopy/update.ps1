...
function global:au_GetLatest {
    ...
    # for 'CharlesLechasseurPathCopyCopy' — verify the filter below still matches what
    # this package actually wants to ship (was: Architecture=x86, InstallerType=Default).
    $releases = "https://evergreen-api.stealthpuppy.com/app/CharlesLechasseurPathCopyCopy"
    $variants = Invoke-RestMethod -Uri $releases -UserAgent "chocolatey-packages-mcp-server"
    $latest = $variants | Where-Object { $_.Architecture -eq 'x86' -and $_.InstallerType -eq 'Default' } | Select-Object -First 1
    if (-not $latest) { throw "No matching evergreen-api variant found for CharlesLechasseurPathCopyCopy." }
    ...
    if ($version -notmatch '^\d+(\.\d+){0,3}(-.+)?$') {
        throw "Sanitized version '$version' has more than 4 numeric segments (NuGet/choco's limit) — adjust this au_GetLatest's version handling for CharlesLechasseurPathCopyCopy's actual version format."
    }
